...
If the API you're calling has an OpenAPI spec, generate the client instead of hand-writing it — the generated functions still send their requests with grab, so everything above applies to every endpoint. See [Set Up Hey API with GRAB](/docs/openapi-services/heyapi-setup).
